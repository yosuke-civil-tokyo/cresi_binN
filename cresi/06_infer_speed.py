...
###############################################################################
def get_patch_lane_singlechannel(patch, window_matrix, thres=4):
    distance_matrix = patch * window_matrix
    distance_matrix[distance_matrix==0] = np.inf
    distance = np.min(distance_matrix)
    distance = min(distance, 15)

    return max(distance, thres)*2


###############################################################################
def get_edge_lane_properties(mask_image, edge_data, conv_dict,
                             min_z=128, dx=4, dy=4, percentile=80,
                             max_speed_band=-2, use_weighted_mean=True,
                             variable_edge_speed=False,
                             verbose=False):
    ...
    # roadarea = 0, other = 1
    # mask_image = mask
    # print('image shape is ', mask_image.shape)
    for j, (x, y, dl_pix) in enumerate(zip(x_mids, y_mids, dls)):
        window_x = (np.arange(-dx-min(0, x-dx), dx-max(0, x+dx+1-mask_image.shape[1])+1) ** 2).reshape((1, -1))
        window_y = (np.arange(-dy-min(0, y-dy), dy-max(0, y+dy+1-mask_image.shape[0])+1) ** 2).reshape((-1, 1))
        window_matrix = np.sqrt(window_x + window_y)
        x0, x1 = max(0, x - dx), x + dx + 1
        y0, y1 = max(0, y - dy), y + dy + 1
        if verbose:
            logger1.info("  x, y, dl: " + str(x), str(y), str(dl_pix))
        ...
        # multichannel case...
        if multichannel:
            patch = mask_image[:, y0:y1, x0:x1]
            nchannels, h, w = mask_image.shape
            if max_speed_band < nchannels - 1:
                patch = patch[:max_speed_band + 1, :, :]
                # # assume the final channel is total, so cut it out
                # nchannels, h, w = mask.shape
            ...
            patch = mask_image[y0:y1, x0:x1]
            length = dl_pix
            lane_pix = get_patch_lane_singlechannel(patch, window_matrix)

        # add to arrays
        lane_arr.append(lane_pix)
        z_arr.append(length)
    ...
    if variable_edge_speed:
        ...
        if multichannel:
            ...
            # or always use variable edge speed?
            mean_lane_pix = np.sum(np.array(lane_arr) * np.array(z_arr) / np.sum(z_arr))
            mean_lane_m = mean_lane_pix * 0.3

            ## get mean of speed_arr
            # mean_speed_mph = np.mean(speed_arr)
            # tot_hours = length_miles / mean_speed_mph

    return mean_lane_m, length_m


def get_both_edge_lane_properties(road_mask_table, edge_data, conv_dict,
                             min_z=128, dx=4, dy=4, percentile=80,
                             max_speed_band=-2, use_weighted_mean=True,
                             variable_edge_speed=False,
                             verbose=False, pix_size=1.4):
    '''
    Get speed estimate from proposal mask and graph edge_data by
    inferring the speed along each segment based on the coordinates in the
    output mask,
    ...
    left_pix = road_mask_table[road_mask_table['roadid']==osmid]
    right_pix = road_mask_table[road_mask_table['roadid']==(-1)*osmid]

    num_left_pix = len(left_pix)
    num_right_pix = len(right_pix)
    ...
    if variable_edge_speed:
        ...
        mean_lane_pix_left = num_left_pix / length_pix
        mean_lane_pix_right = num_right_pix / length_pix
        mean_lane_m_left = num_left_pix * (1.2**2) / length_m
        mean_lane_m_right = num_right_pix * (1.2**2) / length_m
        ## get mean of speed_arr
        # mean_speed_mph = np.mean(speed_arr)
        # tot_hours = length_miles / mean_speed_mph

    return min(mean_lane_m_left, 12), min(mean_lane_m_right, 12), length_m
...
def infer_travel_time(params):
    ...
    G_, road_mask_table, mask, conv_dict, min_z, dx, dy, \
                      percentile, \
    ...
    edge_table = []
    print('{} edges exist'.format(len(G_.edges())))
    mask_image = (mask==0)*1 + (mask==1)*0
    for i,(u, v, edge_data) in enumerate(G_.edges(data=True)):
        if verbose: #(i % 100) == 0:
            logger1.info("\n" + str(i) + " " + str(u) + " " + str(v) + " " \
                         + str(edge_data))
        if (i % 100) == 0:
            print('Now {}th edge'.format(i))
            logger1.info(str(i) + " / " + str(len(G_.edges())) + " edges")
	
        mean_lane_left, mean_lane_right, length_m = \
                get_both_edge_lane_properties(road_mask_table, edge_data, conv_dict,
        ...
        vehicle = 0
        speed = 0
        lane_left = max(1, round(mean_lane_left / 3.5))
        lane_right = max(1, round(mean_lane_right / 3.5))
        osmid = edge_data['osmid']
        edge_table.append([int(osmid), int(u), int(v), int(lane_left), length_m])
        edge_table.append([-int(osmid), int(v), int(u), int(lane_right), length_m])
        
        # update edges
        edge_data['inferred_lane_width'] = np.round(mean_lane_left+mean_lane_right, 2)
        edge_data['length_meter'] = length_m
    ...
    edge_df = pd.DataFrame(edge_table, columns=['edge_id', 'from', 'to', 'Lanes', 'length'])
    edge_df = edge_df[edge_df['from']!=edge_df['to']]
    nodes_data = G_.nodes(data=True)
    nodes_unique = set(edge_df['from'].values) | set(edge_df['to'].values)
    node_table = [[nodes_data[n][col] for col in ['osmid', 'lat', 'lon']] for n in nodes_unique]
    node_df = pd.DataFrame(node_table, columns=['node_id', 'lat', 'lon'])

    edge_df.to_csv('/opt/cresi/results/edge_for_sim.csv', index=False)
    node_df.to_csv('/opt/cresi/results/node_for_sim.csv', index=False)
    print('saved network for sim')
    ...
    if save_shapefiles:
        ...
        out_file = os.path.join(graph_dir_out, im_root + '.shp')
        ox.save_graph_shapefile(G_out, filepath=im_root,
                                encoding='utf-8')
...
###############################################################################
def add_travel_time_dir(graph_dir, result_dir, mask_dir, conv_dict, graph_dir_out,
    ...
    for i,image_name in enumerate(image_names):
        ...
        road_mask_table = pd.read_csv(result_dir+'pix_to_edge.csv')

        mask_path = '/opt/cresi/results/dar_tutorial_cpu/mask_for_lane/'
        mask = skimage.io.imread(mask_path+'mask.tif')
        ...
        params.append((G_raw, road_mask_table, mask, conv_dict, min_z, dx, dy, \
                      percentile, \
                      max_speed_band, use_weighted_mean, \
                      variable_edge_speed, \
                      verbose, \
...
def main():
    ...
    dx, dy = 30, 30   # nearest neighbors patch size  (default = (4, 4))
    ...
    add_travel_time_dir(graph_dir, result_dir, mask_dir, conv_dict, graph_speed_dir,
                      min_z=min_z, 
                      dx=dx, dy=dy,
                      percentile=percentile,
                      max_speed_band=max_speed_band, 
