...
###############################################################################
def contract_short_edges(G_, min_spur_length_m=2, length_key='length',
                       verbose=True):
    """Remove unconnected edges shorter than the desired length"""
    if verbose:
        print("Remove edges shorter than {}".format(min_spur_length_m))
    edge_list = list(G_.edges())
    ...
    for i, (u, v) in enumerate(edge_list):
        ...
        # contract the node with small degree to the other node
        if length < min_spur_length_m:
            degree_u = G_.degree(u)
            degree_v = G_.degree(v)
            if degree_v <= degree_u:
                if v not in remove_nodes:
                    short_edges.append((u, v, length))
                    remove_nodes.append(v)
            else:
                if u not in remove_nodes:
                    short_edges.append((v, u, length))
                    remove_nodes.append(u)

    short_edges = np.array(short_edges)
    if verbose:
        print("num short edges:", print(len(short_edges)))

    for i, (left, right, length) in enumerate(short_edges):
        # if verbose:
        #     print(i, "/", len(edge_list),
        #           "left, right, length:", left, right, length)
        nx.contracted_nodes(G_, left, right, copy=False)
        condition = (short_edges == right)
        short_edges[condition] = left
...
###############################################################################
def make_edge_id(edges):
    table = []
    for osm, (u,v,edge_data) in enumerate(edges):
        linestring = edge_data['wkt_pix']
        line_list = linestring[len('LINESTRING ('):-1].split(', ')
        starts = [line_list[i].split(' ') for i in range(len(line_list)-1)]
        ends = [line_list[i].split(' ') for i in range(1, len(line_list))]
        osmid = edge_data['osmid']
        short_edge_list = [[int(starts[i][1])//4, int(starts[i][0])//4, int(ends[i][1])//4, int(ends[i][0])//4, 0, 0, osm] for i in range(len(starts))]
        for edge_info in short_edge_list:
            table.append(edge_info)

    table = np.array(table)
    table[:, 4] = (table[:, 0] + table[:, 2]) // 2
    table[:, 5] = (table[:, 1] + table[:, 3]) // 2
...
def lines_point_dist(line_starts, line_ends, pt):
    a = line_starts - line_ends
    b = np.tile(pt, (line_starts.shape[0], 1)) - line_ends
    bool_online = 1 * ((np.repeat(0, a.shape[0]) <= dot_prod(a, b)) & (dot_prod(a, b) <= (np.linalg.norm(a,axis=1)**2)))
    dist1 = np.abs(cross_prod(a, b)) / np.linalg.norm(a, axis=1)
    b2 = np.tile(pt, (line_starts.shape[0], 1)) - line_starts
    dist_to_endpoints = np.concatenate([np.linalg.norm(b, keepdims=True, axis=1), np.linalg.norm(b2, keepdims=True, axis=1)], axis=1)
    dist2 = np.min(dist_to_endpoints, axis=1)
    distance = dist1 * bool_online + dist2 * (1 - bool_online)
    return distance


def lr_of_edge(line_starts, line_ends, pts):
    a = line_ends - line_starts
    b = pts - line_starts

    cross = cross_prod(a, b)
    lr = (-2)*np.signbit(cross) + 1

    return lr


##############################################################################
def convert_roadmask(mask_path, edge_data, pix_size=1.4):
    mask_path = '/opt/cresi/results/dar_tutorial_cpu/mask_for_lane/'
    mask_img = cv2.imread(mask_path + 'mask_qua.tif')

    """
    mask_out = mask_img.copy()
    for edge in edge_data:
        mask_out[edge[0], edge[1], 1:] = 0
        mask_out[edge[0], edge[1], 0] = 255
        mask_out[edge[2], edge[3], 1:] = 0
        mask_out[edge[2], edge[3], 0] = 255
    cv2.imwrite(mask_path+'mask_qua_point.tif', mask_out)
    """

    mask_img = cv2.cvtColor(mask_img, cv2.COLOR_BGR2GRAY)
    print('Max of mask : ', np.max(mask_img))
    pix_on_road = np.array(np.where(mask_img>=10)).T
    print('Num of candidate road pix : {}'.format(len(pix_on_road)))
    ...
    n_starts = []
    n_ends = []
    for i in range(w//300+1):
        for j in range(h//300+1):
            pix_bool = np.where((300*i<=pix_on_road[:,0])&(pix_on_road[:,0]<300*(i+1))
                                &(300*j<=pix_on_road[:,1])&(pix_on_road[:,1]<300*(j+1)))
            pix_in_window = pix_on_road[pix_bool].reshape((-1, 2))

            edge_bool = np.where((300*i-100<=edge_data[:,4])&(edge_data[:,4]<=300*i+400)
                                 &(300*j-100<=edge_data[:,5])&(edge_data[:,5]<=300*j+400))
            starts = edge_data[edge_bool, 0:2].reshape((-1, 2))
            ends = edge_data[edge_bool, 2:4].reshape((-1, 2))
            osmid = edge_data[edge_bool, -1].reshape(-1)
            if (len(pix_in_window)==0)|(len(starts)==0):
                continue

            for pix in pix_in_window:
                distance = lines_point_dist(starts, ends, pix)
                nearest_id = np.nanargmin(distance)

                nearest_start = starts[nearest_id]
                nearest_end = ends[nearest_id]
                nearest_osmid = osmid[nearest_id]
                dist_to_nearest = distance[nearest_id] * 1.2
                lane_distributed = 1 + (dist_to_nearest // 3.5)

                if dist_to_nearest<20:
                    n_starts.append(nearest_start)
                    n_ends.append(nearest_end)
                    table.append([pix[0], pix[1], int(nearest_osmid), dist_to_nearest, lane_distributed])
                else:
                    mask_img[pix[0],pix[1]]=0
            print('Now {} {}, {} pix and {} edge'.format(i,j, len(pix_in_window), len(osmid)))

    print('All road pix : {}'.format(len(table)))
    table = np.array(table)
    n_starts = np.array(n_starts)
    n_ends = np.array(n_ends)

    lr = lr_of_edge(n_starts, n_ends, table[:, 0:2])
    table[:, 2] = table[:, 2] * lr

    table = table[table[:,3]<12]
    cv2.imwrite(mask_path+'mask_qua_f.tif', mask_img)
    img_ori = cv2.resize(mask_img, dsize=None, fx=4, fy=4)
    cv2.imwrite(mask_path+'mask_f.tif', img_ori)
...
def wkt_to_G(params):
    ...
    # t31 = time.time()
    # Gout = contract_short_edges(Gout, min_spur_length_m=min_spur_length_m)
    # t32 = time.time()
    # print("Time to contract_short_edges():", t32 - t31, "seconds")
    # ###########################################################################
    ...
    ##if simplify_graph:
      ...
      ##  #   shaply.ops.linemergeconcats 
        ...
        key_tmp = 'geometry_utm_wkt'   # 'geometry_wkt'
        for i,(u,v,attr_dict) in enumerate(Gout.edges(data=True)):
            if verbose and ((i % 10000) == 0):
    ...
    for i, (u, v, edge_data) in enumerate(Gout.edges(data=True)):
        edge_data['osmid'] = i+1

    mask_path = '/opt/cresi/results/dar_tutorial_cpu/mask_for_lane/'
    result_path = '/opt/cresi/results/'
    edge_with_id = make_edge_id(Gout.edges(data=True))
    # create road area data (x, y, osmid_of_edge)
    roadpix_edge = convert_roadmask(mask_path, edge_with_id)
    roadpix_edge = pd.DataFrame(roadpix_edge, columns=['y_pix', 'x_pix', 'roadid', 'dist_from_nearest', 'lane_num'])
    roadpix_edge.to_csv(result_path+'pix_to_edge.csv', index=False)
...
def main():
    ...
    manually_reproject_nodes = True #True
    n_threads = 12
