...
# From road_raster.py
###############################################################################
def dl_post_process_pred(mask, glob_thresh=80, kernel_size=24,
    ...
    print('closing with kernel size:', kernel_close)
...
def preprocess(img, thresh, img_mult=255, hole_size=300,
    ...
    if img.size < 10000000000:
        ...
        img = cv2.dilate(img.astype(np.uint8), np.ones((25, 25)))
        remove_small_objects(img.astype(np.bool), hole_size, in_place=True)
        remove_small_holes(img.astype(np.bool), hole_size, in_place=True)
        # img = cv2.dilate(img.astype(np.uint8), np.ones((25, 25)))
        img = cv2.erode(img.astype(np.uint8), np.ones((25, 25)))
    ...
    #  effective)
        ...
        mask_thresh = glob_thresh_arr_smooth      
        """
        img = (img > (img_mult * thresh)).astype(np.uint8)
        # opening and closing
        # http://docs.opencv.org/3.0-beta/doc/py_tutorials/py_imgproc/py_morphological_ops/py_morphological_ops.html
        #gradient = cv2.morphologyEx(mask_thresh, cv2.MORPH_GRADIENT, kernel)
        print('closing with kernel size:', cv2_kernel_close)
        closing_t = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel_close)
...
def make_skeleton(img_loc, thresh, debug, fix_borders, replicate=5,
    ...
    out_file_mask_norm = '/opt/cresi/results/dar_tutorial_cpu/mask_for_lane/'
    os.makedirs(out_file_mask_norm, exist_ok=True)
    print('img.shape : ')
    img = img.astype(np.uint8)
    img = img * 250
    print(img.shape)
    img_thresh = cv2.resize(img, dsize=None, fx=0.25, fy=0.25)
    print(img.shape)
    #ret, img = cv2.threshold(img, 100, 250, cv2.THRESH_BINARY)
    #ret, img_thresh = cv2.threshold(img_thresh, 100, 250, cv2.THRESH_BINARY)
    print('img.max : ')
    print(np.max(img))
    print(np.sum(img!=0))
    print(np.sum(img_thresh != 0))
    # cv2.imwrite(out_file_mask_norm+'mask.png', cv2.cvtColor(img.astype(np.uint8) * 255, cv2.COLOR_BGR2GRAY))
    cv2.imwrite(out_file_mask_norm+'mask.tif', img)
    cv2.imwrite(out_file_mask_norm + 'mask_qua.tif', img_thresh)
...
###############################################################################
def build_wkt_dir(indir, outfile, out_ske_dir, out_gdir='', thresh=0.15,
...
def main():
    ...
    print('road thresh : {}'.format(thresh))
